module CoPlan
  module Libraries
    ...
    class Organize
      ...
      def self.call(library:, actor:, operations:, actor_type: nil, actor_label: nil, dry_run: false)
        new(library:, actor:, operations:, actor_type:, actor_label:, dry_run:).call
      end

      def initialize(library:, actor:, operations:, actor_type: nil, actor_label: nil, dry_run: false)
        @library = library
        @actor = actor
      ...
      def move_by_tag(op)
        ...
        plans_for_tag(op).find_each do |plan|
          result = Plans::Place.call(
            ...
            actor_type: @actor_type, run_id: @run_id, event_metadata: event_metadata
          )
          if result.success?
            moved << { plan_id: plan.id, plan_title: plan.title }
          else
