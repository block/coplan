...
RSpec.describe Plans::TriggerAutomatedReviews do
  ...
  before do
    ...
    plan.organization.automated_plan_reviewers.destroy_all
  end
  ...
  describe ".call" do
    ...
    it "does not enqueue jobs for reviewers that do not trigger on the status" do
      ...
      expect {
        described_class.call(plan: plan, new_status: "considering", triggered_by: user)
      }.not_to have_enqueued_job(AutomatedReviewJob)
    end
    ...
    it "enqueues multiple jobs when multiple reviewers match" do
      ...
      expect {
        described_class.call(plan: plan, new_status: "considering", triggered_by: user)
      }.to have_enqueued_job(AutomatedReviewJob).exactly(2).times
    end
